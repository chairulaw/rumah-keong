...
Table users {
  ...
  foto_profile blob, // Tambahan: foto profil pengguna
...
Table tokos {
  ...
  no_hp varchar,
  email varchar,
  logo_toko blob, // Tambahan: logo toko
  created_at datetime [default: `CURRENT_TIMESTAMP`],
  updated_at datetime [default: `CURRENT_TIMESTAMP`]
...
Table produks {
  ...
  nama varchar [not null],
  deskripsi text,
  gambar_produk blob, // Diubah: dari varchar ke blob agar gambar disimpan langsung
  harga decimal,
  stok int,
  created_at datetime [default: `CURRENT_TIMESTAMP`],
  updated_at datetime [default: `CURRENT_TIMESTAMP`],
